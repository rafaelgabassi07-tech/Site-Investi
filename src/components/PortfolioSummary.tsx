  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg relative overflow-hidden group">
          ...
          <p className="text-2xl md:text-3xl font-display font-bold text-slate-900 dark:text-white tracking-tighter mb-2">
            R$ {currentTotalValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </p>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Custo: R$ {totalInvested.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}</span>
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg relative overflow-hidden group">
          ...
          <p className={`text-2xl md:text-3xl font-display font-bold tracking-tighter mb-2 ${totalProfit >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
            {totalProfit >= 0 ? '+' : ''}{totalProfitPercentage.toFixed(2)}%
          </p>
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-black uppercase tracking-widest ${totalProfit >= 0 ? 'text-emerald-600/60 dark:text-emerald-500/60' : 'text-red-600/60 dark:text-red-500/60'}`}>
        ...
        <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg relative overflow-hidden group">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Yield On Cost: {dividendYield.toFixed(2)}%</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Allocation Chart */}
        <div id="composicao" className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: 'var(--tooltip-bg)', border: '1px solid var(--tooltip-border)', borderRadius: '12px' }}
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {allocationData.map((item: any, idx: number) => (
              <div key={idx} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-950/50 rounded-lg border border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate max-w-[60px]">{item.name}</span>
                </div>
                <span className="text-[10px] font-black text-slate-900 dark:text-white">{((item.value / currentTotalValue) * 100).toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyDividends}>
                ...
                <XAxis dataKey="date" stroke="#64748b" fontSize={10} tickFormatter={(val) => (typeof val === 'string' ? val.split('-')[1] : '')} />
          ...
          <div className="mt-4 p-3 bg-blue-600/5 border border-blue-500/10 rounded-xl">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Média Mensal</span>
              <span className="text-xs font-black text-slate-900 dark:text-white">R$ {(totalDividends / 12).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>

        {/* Performance Evolution */}
        <div id="evolucao" className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <span className="text-tiny font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Carteira</span>
              </div>
            </div>
          </div>
          <div className="h-48">
          ...
          <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 rounded-xl">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Variação Período</span>
              <span className={`text-xs font-black ${totalProfitPercentage >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
