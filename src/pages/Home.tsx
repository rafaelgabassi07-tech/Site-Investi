...
export default function Home() {
  ...
  // Apply light theme to home page
  useEffect(() => {
  ...
  return (
    <div className="space-y-4 pb-12 max-w-lg mx-auto md:max-w-4xl lg:max-w-5xl">
      ...
      <header className="px-4 pt-8 space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h2 className="text-2xl font-display font-black text-slate-900 tracking-tight italic uppercase">
        ...
        <div className="bg-blue-600/5 border border-blue-600/10 rounded-[2rem] p-8 text-center space-y-3 shadow-sm">
           ...
           <h3 className="text-xl font-display font-black text-slate-900 tracking-tighter uppercase italic">O Futuro dos seus Investimentos</h3>
      ...
      <main className="px-4 space-y-6">
        ...
        <section className="bg-white border border-slate-200 shadow-xl shadow-slate-200/50 rounded-[2rem] p-6 relative overflow-hidden group">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Patrimônio Consolidado</h3>
              <ChevronRight className="icon-xs text-slate-300" />
            </div>
            <button 
          ...
          <div className="flex flex-col gap-1">
            <span className="text-display-lg text-slate-900 leading-none">
              {showValues ? `R$ ${portfolioStats.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'R$ ••••••'}
            </span>
            <div className="flex items-center gap-3 mt-4">
              <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 rounded-full border border-slate-100">
                <Briefcase className="w-3 h-3 text-blue-500" />
                <span className="text-[11px] font-black text-slate-600 uppercase italic">{portfolioStats.count} Ativos</span>
              </div>
              <div className={`flex items-center gap-1 px-3 py-1 rounded-full border ${portfolioStats.change >= 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' : 'bg-red-500/10 border-red-500/20 text-red-600'}`}>
        ...
        <section className="relative">
          <div className="relative flex items-center w-full bg-slate-100 border border-slate-200 rounded-2xl shadow-sm focus-within:bg-white focus-within:border-blue-500/30 transition-all p-2 overflow-hidden group">
            <div className="pl-3 text-slate-400 group-focus-within:text-blue-500">
            ...
            <input 
              ...
              className="flex-1 bg-transparent border-none outline-none py-2 px-3 text-sm text-slate-900 placeholder:text-slate-400 font-bold tracking-tight"
            />
          </div>
               ...
               >
                 {suggestions.map((s, idx) => (
                   <Link 
                     key={s.ticker} 
                     to={`/asset/${s.ticker}`}
                     className={`flex items-center justify-between px-6 py-4 hover:bg-slate-50 transition-all ${selectedIndex === idx ? 'bg-slate-50' : ''}`}
                   >
                     <div className="flex items-center gap-4">
                       <span className="font-display font-black text-slate-900 text-base italic uppercase">{s.ticker}</span>
        ...
        <section className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
           <div className="p-6 border-b border-slate-100 flex items-center gap-3">
             ...
             <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest italic">Rankings de Performance</h3>
           </div>
           ...
           <div className="divide-y divide-slate-100">
              {[
                ...
                <Link key={idx} to={item.to} className="flex items-center justify-between p-5 px-6 hover:bg-slate-50 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 text-slate-400 group-hover:text-blue-500 transition-colors">
           ...
           <Link to="/ranking" className="block p-4 text-center bg-slate-50 hover:bg-slate-100 transition-colors border-t border-slate-100">
        ...
        <section className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
           <div className="p-6 border-b border-slate-100 flex items-center gap-3">
             ...
             <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest italic">Radar do Mercado</h3>
           </div>
           ...
           <Link to="/news" className="block p-4 text-center bg-slate-50 hover:bg-slate-100 transition-colors border-t border-slate-100">
        ...
        <section className="space-y-4 pb-12">
           <h3 className="text-lg font-display font-black text-slate-900 tracking-tight italic uppercase px-2">Explorar Nexus</h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {resourceItems.slice(0, 8).map((item, idx) => (
                <Link key={idx} to={item.to} className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center gap-3 hover:border-blue-500/20 hover:shadow-lg transition-all text-center group">
                  ...
                  <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest group-hover:text-slate-900">{item.label}</span>
                </Link>
              ))}
           </div>
        </section>
